// rd_RenderLayers_IsoscelesFix.jsx
// Code modified by ChatGPT-5
// Date: March 14, 2025
// This is a very useful script in certain situations. But it had been broken for several years due to changes in After Effects and so it wasn't usable. I did some tests and was able to determine what was causing the issue. So I crafted up a text prompt (listed below) and ChatGPT was able to fix the script after a few clarifications.
// ChatGPT Text Prompt: "The following script is not working properly in After Effects 2024 when using the "Layer In/Out Range" option. When I execute the script then it will correctly add items into the render queue, but after rendering out the videos then it's just black video. So I examined the duplicated comps and realized that there are two issues happening: While the duration of the layer range is correct, it is always starting at frame 0 instead of the expected layer in/out range. Also all of the layers are not visible. Can you fix the attached script for me?"
// Although there was no license attached to this script, the original authors website is offline <redefinery.com>, and yet the broken version of the script is freely distributed on <aescripts.com>. That left me in a bit of an awkward situation regarding distribution of the modified version of this script. But the code comment included below from the original author gave a feeling of openness: "I'm just trying to share knowledge with and help out my fellow AE script heads". And so with that in mind, I've decided to distribute this modified version of the script using the GPL-3.0 license.
	...
	function rd_RenderLayers_doRenderLayers(groupObj, doRender)
	{
		...
		for (var i=0; i<selectedLayerIndices.length; i++)
		{
			...
			if (layerRange === 2) { // Layer In/Out Range
			    // Use the layer's own inPoint/outPoint (clamped to comp duration)
			    rangeIn  = (layer.inPoint  < 0) ? 0 : layer.inPoint;
			    rangeOut = (layer.outPoint > compQueued.duration) ? compQueued.duration : layer.outPoint;
    
			    // Set the duplicate comp’s work area so that it starts at 0
			    compQueued.workAreaStart = 0;
			    compQueued.workAreaDuration = rangeOut - rangeIn;
...
*/
			    ...
			    // Shift the rendered layer in the duplicate comp so that its inPoint becomes 0
 			   var queuedLayer = compQueued.layer(layerIndex);
			    queuedLayer.startTime -= rangeIn;
			}
