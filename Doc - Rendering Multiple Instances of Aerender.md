...
- You can string together multiple AE projects so that they render back-to-back. This effective a queue for AE projects, each which contain their own render queues. A queue of queues. This is useful if you have tons of different AE projects that need to be rendered out. Also useful if you have thousands of comps within a single AE project. Also any computer that can render AE can handle this setup since it's just a single instance. For example:
